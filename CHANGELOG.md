...
- Reduced repeated Core wording while retaining the existing activation
  metadata, engineering ownership, risk model, and validation boundaries.
...
- Added Scoville Brainstorm to the optional family composition guide and
  clarified the handoff from divergent exploration to selected engineering.
- Added copy-ready implementation, diagnosis, and patch-review examples.
...
- Explain that activating the Skill adds prompt context and is best suited to
  work where scope control, integrity, risk handling, validation, honest
  evidence, and maintainable source code justify the additional token cost.
